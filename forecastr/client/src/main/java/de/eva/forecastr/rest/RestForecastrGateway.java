...
public final class RestForecastrGateway implements ForecastrGateway {
  ...
  public RestForecastrGateway(
      CommunicationHandler communicationHandler, WebSocketClient webSocketClient) {
  ...
  @Override
  public UserPage userPage(int page, int size) {
    return userHandler.getUserPage(page, size);
  }

  @Override
  public User createUser(String username) {
    return userHandler.createUser(username);
  }

  @Override
  public User updateUser(long userId, String username) {
    return userHandler.updateUser(userId, username);
  }
  ...
  public Balance balance(long userId) {
    return walletHandler.getBalance(userId);
  }

  @Override
  public Balance deposit(long userId, BigDecimal amount) {
    return walletHandler.deposit(userId, amount);
  }

  @Override
  public Balance withdraw(long userId, BigDecimal amount) {
    return walletHandler.withdraw(userId, amount);
  }

  @Override
  public List<Market> feed() {
    return eventHandler.getFeed();
  }

  @Override
  public List<Market> search(String text, String status) {
    return eventHandler.searchEvents(text, status);
  }
  ...
  @Override
  public AutoCloseable liveUpdates(long userId, Consumer<LiveUpdate> updates) {
    return webSocketClient.subscribeToLiveUpdates(userId, updates);
  }
}
